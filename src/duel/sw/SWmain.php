...
class SWmain extends PluginBase {
    ...
    public function onEnable() : void
    {
        ...
        $this->configs = array_map(function($value){ return is_string($value) ? TextFormat::colorize($value) : $value; }, yaml_parse_file($this->getDataFolder() . "duel_configs.yml"));
        $this->lang = array_map([TextFormat::class, "colorize"], yaml_parse_file($this->getDataFolder() . "duel_lang.yml"));
        ...
        $this->getServer()->getCommandMap()->register($this->getName(), new SWcommands("duel", $this));
        ...
        foreach ($this->signs as $xyzworld => $arena_name) {
            if ($arena_name === $arena) {
                ...
                if ($tile instanceof Sign) {
                    $tile->setText(
                        TextFormat::AQUA . "DUEL",
                        TextFormat::BOLD . TextFormat::YELLOW . $arena,
