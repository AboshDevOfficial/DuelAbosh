...
class SWlistener implements Listener {
    ...
    public function onPortalBed(PlayerInteractEvent $event){
        ...
        switch($player->getInventory()->getItemInHand()->getId()){
            case Item::BED:
                Server::getInstance()->getCommandMap()->dispatch($player, "duel quit");
            break;
    ...
    {
        $player = $event->getPlayer();
        if (!$player->isOp() || $event->getLine(0) !== 'duel') {
            return;
        }

        $arena = $event->getLine(1);
        if (!isset($this->plugin->arenas[$arena])) {
            $player->sendMessage(TextFormat::RED . "This arena doesn't exist, try " . TextFormat::GOLD . "/dual create");
            return;
        }

        if (in_array($arena, $this->plugin->signs)) {
            $player->sendMessage(TextFormat::RED . "A sign for this arena already exist, try " . TextFormat::GOLD . "/dual signdelete");
            return;
        }
        ...
        if (!$this->plugin->arenas[$arena]->checkSpawns()) {
            $player->sendMessage(TextFormat::RED . "You haven't configured all the spawn points for this arena, use " . TextFormat::YELLOW . "/dual setspawn");
            return;
        }
    ...
    public function onInteract(PlayerInteractEvent $event) : void
    {
        ...
        if($item->getName() === "§aInformation"){
          $player->sendMessage("§b ﺯﺎﻓ ﻲﻗﺎﺑﻭ ﻦﻴﺒﻋﻻ ﻦﻴﺑ ﻡﺎﻘﺗ ﺔﺒﻌﻟ ﻲﻫ");
        } 
    }
