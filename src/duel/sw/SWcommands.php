        ...
        switch ($cmd = strtolower(array_shift($args) ?? "help")) {
            case "join":
                if (count($args) > 2 || (!($sender instanceof Player) && !isset($args[1]))) {
                    $sender->sendMessage($this->formatUsageMessage("Usage: /duel" . $commandLabel . " join <arena> [PlayerName=YOU]"));
                    return false;
                }
            ...
            default:
                if ($sender->isOp()) {
                    $sender->sendMessage($this->formatUsageMessage("Usage: /duel [join:quit]"));
                } else {
